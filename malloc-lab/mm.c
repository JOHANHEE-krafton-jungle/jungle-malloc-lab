...
static void place(freeListNode *node, size_t size) {
    ...
    if ((prev_size - size) >= (2*DSIZE)) {
        ...
        int nindex = (mm_log2(prev_size - size) > 20) ? 15 : mm_log2(prev_size - size) - 5;
        insert_node(&(free_list_array[nindex]), bp);
    } else {
        // free_list에서 삭제
        delete_node(&(free_list_array[index]), bp);
...
void *mm_malloc(size_t size)
{
    ...
    if (size <= DSIZE) {
        ...
        adjustedSize = WSIZE * ((size + (2*WSIZE) + (WSIZE-1)) / WSIZE); // 16B의 배수로 올림
    }

    // printf("malloc %lu -> %lu ", size, adjustedSize);
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    if (size <= DSIZE) {
        ...
        asize = WSIZE * ((size + (2*WSIZE) + (WSIZE-1)) / WSIZE); // 16B의 배수로 올림
    }
